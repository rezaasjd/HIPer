...
#define MATCH_ELEM(actual, golden, K)                       \
        tolerance > std::abs(actual - golden * K)           \
...
{
    auto total = title.size() % 2 == 0 ? 51 : 50;
    auto hs = static_cast<int>((total - title.size()) / 2);
    auto line = std::string(hs, '-');
    std::cout << line << title << line << std::endl;
}


void save_matrix(std::string filename, float* matrix, int M, int N)
{
    std::ofstream os;
    os.open(filename, std::ios::app | std::ios::binary);
    os.write(reinterpret_cast<char*>(&M), sizeof(int));
    os.write(reinterpret_cast<char*>(&N), sizeof(int));
    for (int i = 0; i < M*N; ++i) {
        os.write(reinterpret_cast<char*>(&matrix[i]), sizeof(float));
    }
}

void load_matrix(std::string filename, float* matrix)
{
    std::ifstream is;
    ...
    for (int i = 0; i < M*N; ++i) {
        is.read(reinterpret_cast<char*>(&matrix[i]), sizeof(float));
    }
}
